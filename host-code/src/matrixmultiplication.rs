    ...
    T: FloatCore + 'static + DeviceRepr + ValidAsZeroBits + Default + Unpin + Display + AddAssign,
{
    ...
    let (kernel, kernel_name) = if (TypeId::of::<T>() == TypeId::of::<f32>()) && use_shared_memory {
        (
            device
                .get_func("kernels", "matrixmultiplication_shared_memory_f32")
                .unwrap(),
            "shared_f32",
        )
        ...
        (
            device
                .get_func("kernels", "matrixmultiplication_f32")
                .unwrap(),
            "simple_f32",
        )
        ...
        (
            device
                .get_func("kernels", "matrixmultiplication_shared_memory_f64")
                .unwrap(),
            "shared_f64",
        )
        ...
        (
            device
                .get_func("kernels", "matrixmultiplication_f64")
                .unwrap(),
            "simple_f64",
        )
    } else {
    ...
    for i in 0..exec_cfg.repititions {
        ...
        unsafe {
            kernel
                .clone()
                .launch(cfg, (&d_input_a, &d_input_b, &mut d_output, n as usize))?
        }
        ...
        for row in 0..n as usize {
            for col in 0..n as usize {
                abs_diff_sum +=
                    (h_a[row * n as usize + col] - kernel_result[row * n as usize + col]).abs();
            }
        }
...
fn main() {
    ...
    dev.load_ptx(
        ...
        "kernels",
        &[
            "matrixmultiplication_f32",
            "matrixmultiplication_f64",
            "matrixmultiplication_shared_memory_f32",
            "matrixmultiplication_shared_memory_f64",
        ],
    )
    .unwrap();
